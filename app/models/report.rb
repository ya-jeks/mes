    ...
    [
      {
        ...
        value: Task.accepted.top.where('tasks.updated_at >= ?', Date.today.beginning_of_week.to_date).map(&:total).reduce(&:+)
      },
      {
        label: 'Отгружено, не оплачено',
        value: Task.delivered.top.where('tasks.updated_at >= ?', Date.today.beginning_of_week.to_date).map(&:total).reduce(&:+)
      },
      {
        label: 'Готово на складе',
        value: Task.finished.top.where('tasks.updated_at >= ?', Date.today.beginning_of_week.to_date).map(&:total).reduce(&:+)
      },
      {
        label: 'Запланировано',
